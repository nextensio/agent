...
:msi
	...
	candle %WIX_CANDLE_FLAGS% -dAGENT_PLAT="%~1" -dProductVersion="0.5" -ext WixUIExtension -ext WixUtilExtension -o "%~1\nxt-win.wixobj" -arch %~3 nxt-win.wxs || goto :error
	...
	echo [+] self-sign the msi package
	SignTool sign /debug /f nxtselfcert.pfx /p Letmein123 /d "nextensio.net" /t http://timestamp.digicert.com /v dist\nextensio-amd64.msi
	goto :eof
