...
if [ "$1" = "ios" ]; then
  ...
  $PROJECT_DIR/../../agent/build_apple.sh ios $PROJECT_DIR/../../
  ...
  $PROJECT_DIR/../../agent/build_apple.sh macosx $PROJECT_DIR/../../
  rm -f $DEST_TARGET/$MACOSX_LIB
  lipo -create $RUST_TARGET/aarch64-apple-darwin/release/$NXT_LIB $RUST_TARGET/x86_64-apple-darwin/release/$NXT_LIB -output $DEST_TARGET/$MACOSX_LIB
