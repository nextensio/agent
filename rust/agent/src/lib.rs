...
fn monitor_parse_pending(
    ...
    for (k, _) in parse_pending.iter_mut() {
        if let Some(f) = flows.get_mut(k) {
            if time_now() > f.creation_time + Duration::from_millis(SERVICE_PARSE_TIMEOUT) {
...
fn monitor_flows(
    ...
    for (k, f) in flows.iter_mut() {
        if time_now() > f.last_active + Duration::from_secs(f.cleanup_after as u64) {
...
fn agent_main_thread(platform: usize, direct: usize) {
    ...
    let mut flow_ager = time_now();
    let mut service_parse_ager = time_now();
    let mut monitor_ager = time_now();
    loop {
        ...
        if time_now() > monitor_ager + Duration::from_secs(2) {
            monitor_onboard(&mut agent);
            monitor_gw(&mut agent, &mut poll);
            monitor_vpnfd(&mut agent, &mut poll);
            monitor_ager = time_now();
        }
        if time_now() > service_parse_ager + Duration::from_millis(SERVICE_PARSE_TIMEOUT) {
            ...
            service_parse_ager = time_now();
        }
        if time_now() > flow_ager + Duration::from_secs(30) {
