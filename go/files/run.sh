...
if [ "$NXT_GW_1_NAME" != "" ] &&  [ "$NXT_GW_2_NAME" != "" ]
then
mkdir /opt/nextensio
...
elif [ "$NXT_GW_1_NAME" != "" ] &&  "$NXT_GW_2_NAME" != "" ]
then
    ...
    # Just launch the connector with the arguments being passed
    2>/var/log/connector.log 1>/var/log/connector.log /go/bin/connector $*&
fi
